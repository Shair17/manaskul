...
import type { Enrollment, Course, Grade, User } from "@prisma/client";
...
export const EditGradesForm: React.FC<Props> = ({ enrollment }) => {
  ...
  const [unit1Score, setUnit1Score] = useState<string>(
    () =>
      enrollment.grades.find((grade) => grade.unit === 1)?.score.toString() ??
      "",
  );
  const [unit2Score, setUnit2Score] = useState<string>(
    () =>
      enrollment.grades.find((grade) => grade.unit === 2)?.score.toString() ??
      "",
  );
  const [unit3Score, setUnit3Score] = useState<string>(
    () =>
      enrollment.grades.find((grade) => grade.unit === 3)?.score.toString() ??
      "",
  );
